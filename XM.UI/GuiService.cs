...
namespace XM.UI
{
        ...
        IAreaEnterEvent
    {
        ...
        private void LoadWindowTemplates()
        {
            ...
            {
                var constructedWindow = definition.BuildWindow();

                // Safety check to ensure we don't try to build the same type of window more than once.
                if (!_windowTemplates.TryAdd(constructedWindow.Type, constructedWindow))
                {
            ...
            _logger.Info($"Loaded {_windowTemplates.Count} GUI window templates.");
        }
        ...
        /// </summary>
        private void CreatePlayerWindows()
        {
            ...
            var dbPlayer = _db.Get<PlayerUI>(playerId) ?? new PlayerUI(playerId);
        ...
        /// </summary>
        private void SavePlayerWindowGeometry()
        {
        ...
        {
            var dbPlayer = _db.Get<PlayerUI>(playerId);
            if (dbPlayer == null)
                return;
        ...
        /// </summary>
        private void HandleNuiWatchEvent()
        {
            var player = NuiGetEventPlayer();
            var playerId = GetObjectUUID(player);
            var windowToken = NuiGetEventWindow();
        ...
        public int ColorTransparent = Convert.ToInt32("0xFFFFFF00", 16);
        public int ColorWhite = Convert.ToInt32("0xFFFFFFFF", 16);
        public int ColorSilver = Convert.ToInt32("0xC0C0C0FF", 16);
        public int ColorGray = Convert.ToInt32("0x808080FF", 16);
        public int ColorDarkGray = Convert.ToInt32("0x303030FF", 16);
        public int ColorBlack = Convert.ToInt32("0x000000FF", 16);
        public int ColorRed = Convert.ToInt32("0xFF0000FF", 16);
        public int ColorMaroon = Convert.ToInt32("0x800000FF", 16);
        public int ColorOrange = Convert.ToInt32("0xFFA500FF", 16);
        public int ColorYellow = Convert.ToInt32("0xFFFF00FF", 16);
        public int ColorOlive = Convert.ToInt32("0x808000FF", 16);
        public int ColorLime = Convert.ToInt32("0x00FF00FF", 16);
        public int ColorGreen = Convert.ToInt32("0x008000FF", 16);
        public int ColorAqua = Convert.ToInt32("0x00FFFFFF", 16);
        public int ColorTeal = Convert.ToInt32("0x008080FF", 16);
        public int ColorBlue = Convert.ToInt32("0x0000FFFF", 16);
        public int ColorNavy = Convert.ToInt32("0x000080FF", 16);
        public int ColorFuschia = Convert.ToInt32("0xFF00FFFF", 16);
        public int ColorPurple = Convert.ToInt32("0x800080FF", 16);

        public int ColorHealthBar = Convert.ToInt32("0x8B0000FF", 16);
        public int ColorEPBar = Convert.ToInt32("0x00008BFF", 16);

        private int ReservedIdCount;

        private readonly Dictionary<string, IdReservation> _systemReservedIdCount = new Dictionary<string, IdReservation>();
