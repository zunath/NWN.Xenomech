...
namespace XM.UI
{
        ...
        where T : IGuiViewModel
    {
        ...
        public GuiWindowBuilder(GuiService gui, IServiceManager serviceManager)
        {
            _gui = gui;
        ...
        {
            _activeWindow = _serviceManager.AnvilServiceContainer.Create(typeof(GuiWindow<T>)) as GuiWindow<T>;
            _type = type;
        ...
        private void RegisterElementEvents(List<IGuiWidget> elements, string windowId)
        {
            foreach (var element in elements)
            {
                foreach (var (eventName, methodDetail) in element.Events)
                {
                    ...
                    {
                        var eventKey = _gui.BuildEventKey(windowId, element.Id);
                        _gui.RegisterElementEvent(eventKey, eventName, methodDetail);
                    }
                }
        ...
        {
            var windowId = _gui.BuildWindowId(_type);
            var windowEventKey = _gui.BuildEventKey(windowId, "_window_");

            // Register window events.
            if(_activeWindow.OpenedEventMethodInfo != null)
                _gui.RegisterElementEvent(windowEventKey, "open", _activeWindow.OpenedEventMethodInfo);

            if(_activeWindow.ClosedEventMethodInfo != null)
                _gui.RegisterElementEvent(windowEventKey, "close", _activeWindow.ClosedEventMethodInfo);
