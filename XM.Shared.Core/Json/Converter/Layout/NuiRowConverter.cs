    ...
    public class NuiRowConverter : JsonConverter<NuiRow>
    {
        public override NuiRow Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    case "aspect":
                        row.Aspect = reader.TokenType == JsonTokenType.Null
                            ? null
                            : reader.GetSingle();
                        break;

                    case "enabled":
                        row.Enabled = JsonSerializer.Deserialize<NuiProperty<bool>>(ref reader, options);
                        break;

                    case "foreground_color":
                        row.ForegroundColor = JsonSerializer.Deserialize<NuiProperty<Color>>(ref reader, options);
                        break;

                    case "height":
                        row.Height = reader.TokenType == JsonTokenType.Null
                            ? null
                            : reader.GetSingle();
                        break;

                    case "id":
                        row.Id = reader.TokenType == JsonTokenType.Null
                            ? null
                            : reader.GetString();
                        break;

                    case "margin":
                        row.Margin = reader.TokenType == JsonTokenType.Null
                            ? null
                            : reader.GetSingle();
                        break;

                    case "padding":
                        row.Padding = reader.TokenType == JsonTokenType.Null
                            ? null
                            : reader.GetSingle();
                        break;

                    case "tooltip":
                        row.Tooltip = JsonSerializer.Deserialize<NuiProperty<string>>(ref reader, options);
                        break;
                    ...
                    case "visible":
                        row.Visible = JsonSerializer.Deserialize<NuiProperty<bool>>(ref reader, options);
                        break;

                    case "width":
                        row.Width = reader.TokenType == JsonTokenType.Null
                            ? null
                            : reader.GetSingle();
                        break;

                    case "draw_list":
                        row.DrawList = JsonSerializer.Deserialize<List<NuiDrawListItem>>(ref reader, options);
                        break;

                    case "draw_list_scissor":
                        row.Scissor = JsonSerializer.Deserialize<NuiProperty<bool>>(ref reader, options);
                        break;

                    case "disabled_tooltip":
                        row.DisabledTooltip = JsonSerializer.Deserialize<NuiProperty<string>>(ref reader, options);
                        break;

                    case "encouraged":
                        row.Encouraged = JsonSerializer.Deserialize<NuiProperty<bool>>(ref reader, options);
                        break;

                    case "children":
                        row.Children = JsonSerializer.Deserialize<List<NuiElement>>(ref reader, options) ?? new List<NuiElement>();
                        break;
